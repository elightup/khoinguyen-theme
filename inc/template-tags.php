	...
	<div class="category">
		<div class="category-menu">
			<div class="category-menu-icon">
				<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/menu.png" alt="">
			</div>
						...
						<div class="filter-category__children d-flex">
							...
							foreach ( $childrens as $children ) :
								...
								<div class="category__children-item d-flex">
									...
									<?php endif; ?>
									<a href="<?php echo esc_url( get_term_link( $term_child->slug, 'nganh-hang' ) ); ?>"><?php echo esc_html( $term_child->name ); ?></a>
								</div>
							<?php endforeach; ?>
						</div>
						<svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="angle-right" class="svg-inline--fa fa-angle-right fa-w-8" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 512"><path fill="currentColor" d="M224.3 273l-136 136c-9.4 9.4-24.6 9.4-33.9 0l-22.6-22.6c-9.4-9.4-9.4-24.6 0-33.9l96.4-96.4-96.4-96.4c-9.4-9.4-9.4-24.6 0-33.9L54.3 103c9.4-9.4 24.6-9.4 33.9 0l136 136c9.5 9.4 9.5 24.6.1 34z"></path></svg>
					<?php endif; ?>
					<a href="<?php echo esc_url( get_term_link( $term->slug, 'nganh-hang' ) ); ?>"><?php echo esc_html( $term->name ); ?></a>
				</li>
				<?php endforeach; ?>
			</ul>
		</div>
	...
	<div class="filter-category">
		<ul>
			...
			foreach ( $terms as $key => $term_id ) :
				...
				?>
			<li>
				<a href="<?php echo esc_url( get_term_link( $term_id, 'nganh-hang' ) ); ?>"><?php echo esc_html( $term->name ); ?></a>
			</li>
			<?php endforeach; ?>
		</ul>
	</div>
...
function kn_filter_nganh_hang() {
	...
	$selected = isset( $_GET['filter-nganh-hang'] ) ? wp_strip_all_tags( wp_unslash( $_GET['filter-nganh-hang'] ) ) : '';
	?>
...
function kn_filter_sap_xep() {
	$selected = isset( $_GET['filter-sap-xep'] ) ? wp_strip_all_tags( wp_unslash( $_GET['filter-sap-xep'] ) ) : '';
	?>
...
function kn_filter_gia() {
	$selected = isset( $_GET['filter-gia'] ) ? wp_strip_all_tags( wp_unslash( $_GET['filter-gia'] ) ) : '';
	?>
...
function kn_filter_hang() {
	...
	$selected = isset( $_GET['filter-hang'] ) ? wp_strip_all_tags( wp_unslash( $_GET['filter-hang'] ) ) : '';
	?>
...
function kn_custom_filter() {
	...
	foreach ( $custom_filter as $filter ) :
		$selected = isset( $_GET[ 'filter-' . $filter ] ) ? wp_strip_all_tags( wp_unslash( $_GET[ 'filter-' . $filter ] ) ) : '';
		?>
		<select name="filter-<?php echo esc_attr( $filter ) ?>">
			<option value=""><?php echo esc_html( $total_filter[ $filter ]['name'] ); ?></option>
		...
		<div class="seclect-product-list">
			...
			<div class="product-lists">
				...
				<div class="product_item" id="product" data-title="<?php the_title(); ?>"
					data-id="<?php echo get_the_ID() ?>" data-link="<?php echo esc_attr( admin_url( 'admin-ajax.php' ) ) ?>">
...
function load_sosanh( $id ) {
	...
	while ( $query->have_posts() ) :
		...
		?>
	<div class="filter-product-content" data-name="<?php echo esc_attr( get_the_title() ) ?>">
		<div class="filter-product-top <?php echo esc_attr( $lable ) ?>">
			<div class="box_image">
			...
			<div class="box_product-datmua">
				...
				if ( in_array( get_the_ID(), $cart_product_id ) ) :
					?>
						...
						<a href="<?= esc_url( home_url() ); ?>/gio-hang" class="btn btn-muangay" data-product="<?= esc_attr( $product_id ); ?>">Mua
							ngay </a>
					<?php else : ?>
						<a href="#" class="btn btn-them single-add-to-cart"
							data-info="<?= esc_attr( wp_json_encode( kn_get_product_info( $product_id ) ) ); ?>"
							data-product="<?= esc_attr( $product_id ); ?>">Thêm vào giỏ hàng</a>
						<a href="#" class="btn btn-muangay single-buynow"
							data-info="<?= esc_attr( wp_json_encode( kn_get_product_info( $product_id ) ) ); ?>"
							data-product="<?= esc_attr( $product_id ); ?>">Mua ngay</a>
						<?php
					endif;
					?>
			</div>

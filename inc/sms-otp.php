...
function kn_check_otp( $user ) {
	...
	if ( empty( $otp_code ) ) {
		$text = 'Tài khoản của bạn chưa được xác thực! Bấm <a href="' . esc_url( home_url() ) . '/nhap-ma-otp?user_id=' . $user->ID . '"> vào đây </a> rồi ấn gửi lại mã để nhập mã OTP để nhập mã OTP';
